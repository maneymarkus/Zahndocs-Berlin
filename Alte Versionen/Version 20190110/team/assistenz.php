<!doctype html>
<html lang="de">
	<head>
		<meta charset="UTF-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=yes" />
		<meta name="keywords" content="Zahnarzt, Zahnärzte, Zahnatztpraxis, Berlin, Hellerdsdorf, Marzahn, Helle-Mitte, Gemeinschaftspraxis, Katrin, Kunath, Dornberger, Schmidt, Al-Hasaki, Implantat, Implantologie, Brücken, Prothese, Zahnschmerzen, Notfall, Notdienst, Narkosebehandlung, Narkose, Bleeching, weiße Zähne, Angstbehandlung, Schmerzpatienten, Interdisziplinäre Sprechstunden, Russisch, Ästehetik, Prophylaxe, Operation, Weisheitszähne, Paradontose, Putzen, Zahnbürste, Zahnärzte in Berlin, Assistenz, Assistentin">
		<meta name="descritpion" content="Während Sie von den Zahnärzten behandelt werden, stehen die Assistentinnen zur Seite, die sich hier kurz vorstellen." />
	  
		<title>Assistenz - Zahndocs-Berlin</title>
	  
		<link rel="icon" href="../images/favicon.ico" type="image/x-icon" />
		<link rel="pingback" href="http://zahndocs-berlin.de/xmlrpc.php" />
		<link rel="canonical" href="http://zahndocs-berlin.de/">
		
		<link href="https://fonts.googleapis.com/css?family=Raleway" rel="stylesheet">
		<link rel="stylesheet" href="https://fonts.googleapis.com/icon?family=Material+Icons">
		<link rel="stylesheet" href="../style/normalize.css" type="text/css" />
		<link rel="stylesheet" href="../style/general-style.css" type="text/css" />
		<link rel="stylesheet" href="team-unterseiten-style.css" type="text/css" />
		
		<script type="text/javascript">
			var gaProperty = 'UA-112741886-1';
			var disableStr = 'ga-disable-' + gaProperty;
			if (document.cookie.indexOf(disableStr + '=true') > -1) {
			window[disableStr] = true;
			}
			function gaOptout() {
			document.cookie = disableStr + '=true; expires=Thu, 31 Dec 2099 23:59:59 UTC;path=/';
			window[disableStr] = true;
			}
		</script>
		
		<!-- Global site tag (gtag.js) - Google Analytics -->
		<script async src="https://www.googletagmanager.com/gtag/js?id=UA-112741886-1"></script>
		<script>
			window.dataLayer = window.dataLayer || [];
			function gtag(){dataLayer.push(arguments);}
			gtag('js', new Date());

			gtag('config', 'UA-112741886-1', { 'anonymize_ip': true });
		</script>
		
		<script src="../script/script.js" type="application/javascript" defer></script>
		<!--[if lt IE 9]>
			<script src="../script/html5shiv.js"></script>
		<![endif]-->
		
		<script src="https://dr-flex.de/embed.js?medicalPracticeId=11731" defer></script>
		
		<!--
		
			Webdesign by Markus Städler, weberlin-design.de, markus.staedler@weberlin-design.de
		
		-->

	</head>
	<body>
		
		<div id="wrapper">
		
			<?php include("/var/www/vhosts/zahndocs-berlin.de/httpdocs/header.html"); ?>
			
			<main>
				
				<ul class="breadcrumb">
					
					<li>
						<span>Sie sind hier:</span>
					</li>
					<li>
						<span>→</span>
						<a href="../index.php">Home</a>
					</li>
					<li>
						<span>→</span>
						<a href="../team.php">Team</a>
					</li>
					<li>
						<span>→</span>
						<span>Assistenz</span>
					</li>
				
				</ul>
				
				<section>
				
					<h2>Assistentinnen</h2>
				
					<div>
					
						<div>
						
							<img src="images/homepage-2017-1-200x300.jpg" alt="Assistentin Irina Miller" />
						
							<h3>Irina Miller (zur Zeit im Mutterschutz)</h3>
						
						</div>
					
						<ul>
						
							<li>Ausbildung zur zahnmedizinischen Fachangestellten 2009-2012</li>
						
						</ul>
						
						<div class="under-image-2">
						
							<h4>Tätigkeit:</h4>
						
							<ul>
						
								<li>Stuhlassistenz</li>
								
							</ul>
						
						</div>
					
					</div>
					
					<hr />
					
					<div>
						
						<div>
						
							<img src="images/homepage-684x1024.jpg" alt="Assistentin Daniela Ardelt" />
						
							<h3>Daniela Ardelt</h3>
						
						</div>
					
						<ul>
						
							<li>Ausbildung zur zahnmedizinischen Fachangestellten 2003-2006</li>
						
						</ul>
						
						<div class="under-image-2">
						
							<h4>Tätigkeit:</h4>
						
							<ul>
						
								<li>Stuhlassistenz</li>
								
							</ul>
						
						</div>
					
					</div>
					
					<hr />
					
					<div>
					
						<div>
						
							<img src="images/2017-03-31Zahndocs-1031.jpg" alt="Assistentin Jean Münchberg" style="width: 100%; max-width: 500px;" />
						
							<h3>Jean Münchberg</h3>
						
						</div>
					
						<ul>
						
							<li>Ausbildung zur zahnmedizinischen Fachangestellten 2009-2012</li>
						
						</ul>
						
						<div class="under-image-2">
						
							<h4>Tätigkeit:</h4>
						
							<ul>
						
								<li>Stuhlassistenz</li>
								
							</ul>
						
						</div>
						
					</div>

                    <hr />

                    <div>

                        <div>

                            <img src="images/1-1804034-platzhalter-f-uuml-r-foto.jpg" alt="Assistentin Ina Wenzel" style="width: 100%; max-width: 300px;" />

                            <h3>Ina Wenzel</h3>

                        </div>

                        <ul>

                            <li>Ausbildung zur stomatologischen Schwester 1980-1983</li>

                        </ul>

                        <div class="under-image-2">

                            <h4>Tätigkeit:</h4>

                            <ul>

                                <li>Stuhlassistenz</li>

                            </ul>

                        </div>

                    </div>

                    <hr />

                    <div>

                        <div>

                            <img src="images/1-1804034-platzhalter-f-uuml-r-foto.jpg" alt="Assistentin Janin Demczynski" style="width: 100%; max-width: 300px;" />

                            <h3>Janin Demczynski</h3>

                        </div>

                        <ul>

                            <li>Ausbildung zur zahnmedizinischen Fachangestellten 2013-2016</li>

                        </ul>

                        <div class="under-image-2">

                            <h4>Tätigkeit:</h4>

                            <ul>

                                <li>Stuhlassistenz</li>

                            </ul>

                        </div>

                    </div>
					
				</section>
			
				<?php include("/var/www/vhosts/zahndocs-berlin.de/httpdocs/aside.html"); ?>
				
				<a id="back-to-top">
					<i class="material-icons">arrow_upward</i>
				</a>
			
			</main>
			
			<?php include("/var/www/vhosts/zahndocs-berlin.de/httpdocs/footer.php"); ?>
		
		</div>
		
		<?php include("/var/www/vhosts/zahndocs-berlin.de/httpdocs/extensions.html"); ?>
		
	</body>
</html>